    ...
    public class VolunteerHomapageOverview
    {
        ...
        private Panel _getVolunteerShiftPanel(Point location, Size size, Worker worker)
        {
            ...
            if (worker != null)
            {
                ...
                List<Shift> ListOfWorkerListOfShifts = volunteerMainUI.GetScheduleController().GetAllListOfShifts().Where(x => x.ListOfWorkers.Exists(y => y == worker)).ToList();
                int widthOfShiftElement = 100;
                for (int i = 0; i < ListOfWorkerListOfShifts.Count; i++)
                {
                    volunteerShiftPanel.Controls.Add(_getSingleShiftPanel(new Point((i * widthOfShiftElement), shiftLabel.Size.Height - 2), new Size(widthOfShiftElement, volunteerShiftPanel.Size.Height - 50), ListOfWorkerListOfShifts[i]));
                }
            }
